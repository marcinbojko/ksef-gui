...
class Program {
  ...
  static int RunGetInvoice(GetInvoiceOptions opts)
  {
      return AsyncRunner.Run(async () =>
      {
          ...
          try
          {
              string invoice = await ksefClient.GetInvoiceAsync(opts.KsefNumber, opts.Token, CancellationToken.None);
  ...
  static int RunExportInvoices(ExportInvoicesOptions opts)
  {
      return AsyncRunner.Run(async () =>
      {
          ...
          try
          {
              ...
              ICryptographyService cryptographyService = new CryptographyService(certificate);
    ...
    static int RunGetExportStatus(GetExportStatusOptions opts)
  {
      return AsyncRunner.Run(async () =>
      {
          IKSeFClient ksefClient = CreateKSeFClient(opts.BaseUrl);
